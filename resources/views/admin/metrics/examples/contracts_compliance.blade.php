            ...
            <tr>
                ...
                <td class="py-3">
                    <div class="progress mb-2 mt-1" style="height: 6px;">
                        <div class="progress-bar" role="progressbar" style="width: {{round((($contract->nodes)/(collect(json_decode($contract->compliance))->count()))*100,2)}}%;" aria-valuenow="{{round((($contract->nodes)/(collect(json_decode($contract->compliance))->count()))*100,2)}}"
                            aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                    <p class="text-uppercase text-muted mb-0 font-size-11">
                        {{((json_decode($contract->compliance))[$contract->nodes])->content}}
                    </p>
                </td>
            </tr>
            @endforeach
