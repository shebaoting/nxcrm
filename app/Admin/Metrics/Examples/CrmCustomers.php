...
class CrmCustomers extends Bar
{
    ...
    protected function init()
    {
        ...
        $customer = DB::table('crm_customers')->where('state', '=', 3);
...
HTML
     ...
     */
     ...
     public function grow (){
        $origin = DB::table('crm_customers')->selectRaw('DATE_FORMAT(created_at,"%Y-%m") as date,COUNT(*) as value')
        ->whereMonth('created_at', date('m'))
        ->groupBy('date')
        ->get()
        ->toArray();

        $last_month = DB::table('crm_customers')->selectRaw('DATE_FORMAT(created_at,"%Y-%m") as date,COUNT(*) as value')
