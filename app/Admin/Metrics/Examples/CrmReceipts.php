...
class CrmReceipts extends Line
{
    /**
     * 初始化卡片内容
     */
    protected function init()
    {
        ...
        $this->num = DB::table('crm_receipts')->selectRaw('DATE_FORMAT(updated_at,"%Y-%m") as date,SUM(receive) as value')->groupBy('date')->orderBy('date')->limit(12)->get();

        $this->origin = DB::table('crm_receipts')->selectRaw('DATE_FORMAT(updated_at,"%Y-%m") as date,SUM(receive) as value')
            ->whereMonth('updated_at', date('m'))
            ->groupBy('date')
            ->get();
            // ->toArray();

        $this->last_month = DB::table('crm_receipts')->selectRaw('DATE_FORMAT(updated_at,"%Y-%m") as date,SUM(receive) as value')
